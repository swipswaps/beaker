...
var RecipeTaskSummary = Backbone.View.extend({
    ...
    render: function () {
        ...
        if (status == 'Cancelled' || status == 'Aborted') {
            ...
            this.$('.recipe-task-status').text(status);
        }
...
var RecipeTaskResults = Backbone.View.extend({
    ...
    render: function () {
        var all_results = _.size(this.model.get('results'));
        this.$el.html(this.template(_.extend({all_results: all_results},
                this.model)));
        var $el = this.$el;
        var recipe_start_time = this.recipe_start_time;
        var model = this.model;
